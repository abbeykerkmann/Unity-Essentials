...
{
    /// <summary>
    /// Custom editor for parts
    /// </summary>
    [CustomEditor(typeof(Part)), CanEditMultipleObjects]
    internal class PartEditor : Editor
    {
        ...
        private void DrawMouldingColour(SerializedProperty property, GUIContent label)
        {
            ...
            else
            {
                ...
                {
                    EditorGUILayout.HelpBox($"{label.text} is a legacy colour", MessageType.Warning);
                }
            }
                ...
                if (position.Contains(Event.current.mousePosition))
                {
                    MouldingColourPicker.Show((c) =>
                    {
                        ...
                        foreach (var target in targets)
                        {
                            ...
                            PartImporter.SetMaterials(partTarget, materials, legacyProp.boolValue);
