...
{
    /// <summary>
    /// Custom editor for <see cref="ModelGroup"/>
    /// </summary>
    [CustomEditor(typeof(ModelGroup), isFallback = true)]
    public class ModelGroupEditor : Editor
    {
        ModelGroup modelGroup;
        SerializedProperty processedProp;
        SerializedProperty viewBasedOptimizationsProp;
        ...
        /// <summary>
        /// Override to write custom code when OnEnable is called
        /// </summary>
        protected virtual void OnEnable()
        {
            modelGroup = (ModelGroup)target;
            processedProp = serializedObject.FindProperty("processed");
            viewBasedOptimizationsProp = serializedObject.FindProperty("viewBasedOptimizations");
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                EditorGUILayout.HelpBox("Combine the model group into a few optimized meshes.\nThe model group can be further optimized based on specified views.", MessageType.Info);

                EditorGUIUtility.labelWidth = 160f;
                EditorGUILayout.PropertyField(viewBasedOptimizationsProp);
                EditorGUIUtility.labelWidth = 0f;

                // Backface culling and geometry removal UI, and sorting UI.
                if (((ModelGroup.ViewBasedOptimizations)viewBasedOptimizationsProp.intValue & (ModelGroup.ViewBasedGeometryRemovalOptimizations | ModelGroup.ViewBasedOptimizations.SortFrontToBack)) != 0)
                {
                    var geometryRemoval = ((ModelGroup.ViewBasedOptimizations)viewBasedOptimizationsProp.intValue & ModelGroup.ViewBasedGeometryRemovalOptimizations) != 0;
                    var sorting = ((ModelGroup.ViewBasedOptimizations)viewBasedOptimizationsProp.intValue & ModelGroup.ViewBasedOptimizations.SortFrontToBack) != 0;
                    ...
                    {
                        EditorGUILayout.PropertyField(viewsProp);
                    }
                }
