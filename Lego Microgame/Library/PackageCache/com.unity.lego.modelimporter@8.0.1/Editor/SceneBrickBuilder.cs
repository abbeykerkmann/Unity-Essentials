...
{
    [InitializeOnLoad]
    internal class SceneBrickBuilder
    {
        ...
        static void OnImportStarted()
        {
            DisableConnectivityEvents();
        }

        static void OnImportEnded()
        {
            sceneChanged = true;
            SyncSceneBricks();
            SetupConnectivityEvents();
        ...
        static SceneBrickBuilder()
        {
            ...
            ModelImporter.modelImportEnded -= OnImportEnded;
            ModelImporter.modelImportEnded += OnImportEnded;

            ModelImporter.modelImportStarted -= OnImportStarted;
            ModelImporter.modelImportStarted += OnImportStarted;

            PartImporter.partImportStarted -= OnImportStarted;
            PartImporter.partImportStarted += OnImportStarted;

            PartImporter.partImportEnded -= OnImportEnded;
            PartImporter.partImportEnded += OnImportEnded;
        ...
        private static HashSet<Brick> RecomputeConnectionsAfterNudgeRotate(IEnumerable<Brick> previouslyColliding)
        {
            ...
            if (BrickBuildingUtility.Colliding(selectedBricks, selectionBounds, focusBrick.gameObject.scene.GetPhysicsScene(), BrickBuildingUtility.ColliderBuffer, out _))
            {
                foreach (var brick in selectedBricks)
                {
        ...
        private static void PlaceBrick()
        {
            ...
            if (!BrickBuildingUtility.Colliding(selectedBricks, selectionBounds, focusBrick.gameObject.scene.GetPhysicsScene(), BrickBuildingUtility.ColliderBuffer, out _, true))
            {
                foreach (var brick in selectedBricks)
                {
                    brick.UpdateColliding(false);
                            ...
                            foreach (var connection in pf.connections)
                            {
                                ...
                                if (connection.HasConnection())
                                {
                                    ...
                                    if(connectionInField.field != pf)
                                    {
                                        ...
                                        pf.connected.Remove(connection.index);
...
#region event handling
        ...
        private static void ComputeNewConnection(Camera camera, Ray ray)
        {
            ...
            if (BrickBuildingUtility.FindBestConnection(pickupOffset, selectedBricks, ray, camera.transform.localToWorldMatrix, bricks, selectionBounds, out BrickBuildingUtility.ConnectionResult[] newResult, ToolsSettings.MaxTriesPerBrick))
            {
            ...
            var isColliding = BrickBuildingUtility.Colliding(selectedBricks, selectionBounds, focusBrick.gameObject.scene.GetPhysicsScene(), BrickBuildingUtility.ColliderBuffer, out _, true);
