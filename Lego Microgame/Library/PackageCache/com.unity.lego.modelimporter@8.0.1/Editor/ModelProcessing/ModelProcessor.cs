...
{
    /// <summary>
    /// Provides methods for processing and optimizing models
    /// </summary>
    public static class ModelProcessor
    {
        private static Mesh GetOptimizedMesh(Mesh source, int optimizationLevel, int lod)
        {
            ...
            Mesh m = AssetDatabase.LoadAssetAtPath<Mesh>(Path.Combine(assetFolder, lodDir, source.name + assetExt));
            if (m)
            {
                return m;
            }
        ...
        static readonly int tubePixelThreshold = 20;

        /// <summary>
        /// Process a single model group
        /// </summary>
        /// <param name="group">The model group to process</param>
        /// <param name="vertCount">Reference parameter specifying input vertex count in x and output vertex count in y</param>
        /// <param name="triCount">Reference parameter specifying input triangle count in x and output triangle count in y</param>
        /// <param name="meshCount">Reference parameter specifying input mesh count in x and output mesh count in y</param>
        /// <param name="boxColliderCount">Reference parameter specifying input box collider count in x and output box collider count in y</param>
        public static void ProcessModelGroup(ModelGroup group, ref Vector2Int vertCount, ref Vector2Int triCount, ref Vector2Int meshCount, ref Vector2Int boxColliderCount)
        {
            ...
            bool doSort = group.viewBasedOptimizations.HasFlag(ModelGroup.ViewBasedOptimizations.SortFrontToBack);
            bool canRemoveInsides = group.viewBasedOptimizations.HasFlag(ModelGroup.ViewBasedOptimizations.RemoveInvisibleInsides);
            bool canRemoveOutsides = group.viewBasedOptimizations.HasFlag(ModelGroup.ViewBasedOptimizations.RemoveInvisibleOutsides);
            bool cullBackfaces = group.viewBasedOptimizations.HasFlag(ModelGroup.ViewBasedOptimizations.BackfaceCulling);
            ...
            foreach (var part in parts)
            {
                ...
                else
                {
                    ...
                    foreach (var renderer in part.GetComponentsInChildren<MeshRenderer>(true))
                    {
                        ...
                        // Then check if it is on a deactivated game object as we will not include deactivated knobs and tubes in the processing.
            ...
            for (int i = 0; i < mrs.Count; ++i)
            {
                ...
                if (mf)
                {
                    if (mrs[i].rendererer.enabled && mrs[i].rendererer.gameObject.activeInHierarchy)
                    {
                        ...
                        if (source)
                        {
                            ...
                            c.normalMap = mf.sharedMesh.uv.Length > 0;
                            c.part = mrs[i].part;
                            c.type = mrs[i].type;
                ...
                Undo.DestroyObjectImmediate(mrs[i].rendererer);
            }
            ...
            {
                if (canRemoveInsides || canRemoveOutsides || cullBackfaces)
                {
                    Debug.LogError("No views specified for backface culling and geometry removal. Disabling!");
                    canRemoveInsides = false;
                    canRemoveOutsides = false;
                    cullBackfaces = false;
                }
            }
            ...
            AnalyzeMeshes(viewMatrices, projectionMatrices, viewPositions, viewDirections, viewFrustums, instances, partMeshCount, partSurfaceMeshCount, canRemoveInsides, canRemoveOutsides, group.importSettings.lod);
            ...
            for (int i = 0; i < instances.Count; ++i)
            {
                ...
                int subMesh = (instances[i].transparent ? 2 : 0) + (instances[i].normalMap ? 1 : 0);
            ...
            for (int i = 0; i < meshHelpers.Length; ++i)
            {
                if (meshHelpers[i].vertices.Count > 0)
                {
                    ...
                    if (i > 0)
                    {
                        ...
                        Undo.SetTransformParent(target.transform, group.transform, false, "Parent sub mesh");
                    }

                    MeshFilter mf = Undo.AddComponent<MeshFilter>(target);
                    MeshRenderer mr = Undo.AddComponent<MeshRenderer>(target);

                    Undo.RecordObject(mf, "Assign sub mesh");
                    ...
                    case 0:
                        {
                            PartUtility.StoreOptimizedMesh(m, group.parentName + "_" + group.groupName + "_Optimized_Fast.asset");
                            mr.sharedMaterial = AssetDatabase.LoadAssetAtPath<Material>("Packages/com.unity.lego.modelimporter/Materials/LEGO_VertexColor_Fast.mat");
                            break;
                        }
                    case 1:
                        {
                    ...
                    case 2:
                        {
                            PartUtility.StoreOptimizedMesh(m, group.parentName + "_" + group.groupName + "_Optimized_Transparent_Fast.asset");
                            mr.sharedMaterial = AssetDatabase.LoadAssetAtPath<Material>("Packages/com.unity.lego.modelimporter/Materials/LEGO_VertexColor_Transparent_Fast.mat");
                            break;
                        }
                    case 3:
                        {
            ...
            // Move active decoration surfaces for non-legacy parts that have any surface meshes left.
            GameObject decorationSurfacesParent = null;
            progress = 0;
            foreach (var entry in partSurfaceMeshCount)
            {
                if (progress++ % 200 == 0)
                {
                    EditorUtility.DisplayProgressBar("Processing", "Moving needed decorations.", 0.3f + (float)progress / partSurfaceMeshCount.Count * 0.25f);
                }
                ...
                if (!entry.Key.legacy && entry.Value > 0)
                {
                    ...
                    {
                        var decorationSurfaceRenderers = decorationSurfaces.GetComponentsInChildren<MeshRenderer>();
                        foreach (var decorationSurfaceRenderer in decorationSurfaceRenderers)
                        {
                            if (decorationSurfaceRenderer.gameObject.activeInHierarchy)
                            {
                                if (!decorationSurfacesParent)
                                {
                                    decorationSurfacesParent = new GameObject("DecorationSurfaces");
                                    Undo.RegisterCreatedObjectUndo(decorationSurfacesParent, "Create decoration surfaces");
                                    Undo.SetTransformParent(decorationSurfacesParent.transform, group.transform, false, "Parent decoration surfaces");
                                }

                                Undo.SetTransformParent(decorationSurfaceRenderer.transform, decorationSurfacesParent.transform, "Move decoration surface");
                            }
                        }
                    }
                }
            ...
            if (collapseCols)
            {
                ...
                var boxColliders = group.GetComponentsInChildren<BoxCollider>();
                // Filter out colliders that are part of connectivity features.
                var partBoxColliders = boxColliders.Where((c) => c.gameObject.layer != LayerMask.NameToLayer(BrickBuildingUtility.ConnectivityReceptorLayerName) && c.gameObject.layer != LayerMask.NameToLayer(BrickBuildingUtility.ConnectivityConnectorLayerName)).ToArray();
                boxColliderCount.x = partBoxColliders.Length;
                boxColliderCount.y = partBoxColliders.Length;
                bool[] colDeleted = new bool[partBoxColliders.Length];
                ...
                while (collapseHappened)
                {
                    ...
                    iterationCount++;
                    for (var i = 0; i < partBoxColliders.Length; ++i)
                    {
                        if (i % 200 == 0)
                        {
                            EditorUtility.DisplayProgressBar("Processing", "Collapsing colliders - iteration " + iterationCount + ".", 0.8f + (float)i / partBoxColliders.Length * 0.05f);
                        }
                        ...
                        for (var j = i + 1; j < partBoxColliders.Length; ++j)
                        {
                            if (colDeleted[j])
                            {
                                continue;
                            ...
                            var colliderA = partBoxColliders[i];
                            var colliderB = partBoxColliders[j];
                                ...
                                if ((centerDiff - centerProjectedToAxis).sqrMagnitude < 0.01f)
                                {
                                    ...
                                    if (Mathf.Abs(centerDiff.magnitude - (colliderA.size[axisToMatch] + colliderBSizeInColliderALocalSpace[axisToMatch] + 2.0f * colliderSizeBias) * 0.5f) < epsilon)
                                    {
                                        ...
                                        if (Mathf.Abs(colliderA.size[otherAxis1] - colliderBSizeInColliderALocalSpace[otherAxis1]) < epsilon && Mathf.Abs(colliderA.size[otherAxis2] - colliderBSizeInColliderALocalSpace[otherAxis2]) < epsilon)
                                        {
                                            ...
                                            part.colliders.colliders.Remove(colliderB);
            ...
            // Move colliders.
            GameObject collidersParent = null;
            progress = 0;
            var allColliders = group.GetComponentsInChildren<Collider>();
            // Filter out colliders that are part of connectivity features.
            var partColliders = allColliders.Where((c) => c.gameObject.layer != LayerMask.NameToLayer(BrickBuildingUtility.ConnectivityReceptorLayerName) && c.gameObject.layer != LayerMask.NameToLayer(BrickBuildingUtility.ConnectivityConnectorLayerName)).ToArray();
            foreach (var partCollider in partColliders)
            {
                if (progress++ % 200 == 0)
                {
                    EditorUtility.DisplayProgressBar("Processing", "Moving colliders.", 0.85f + (float)progress / partColliders.Length * 0.05f);
                }

                if (!collidersParent)
                {
                    collidersParent = new GameObject("Colliders");
                    Undo.RegisterCreatedObjectUndo(collidersParent, "Create colliders");
                    Undo.SetTransformParent(collidersParent.transform, group.transform, false, "Parent colliders");
                }

                Undo.SetTransformParent(partCollider.transform, collidersParent.transform, "Move collider");
            }
            ...
            foreach (var brick in bricks)
            {
                if (progress++ % 200 == 0)
                {
                    EditorUtility.DisplayProgressBar("Processing", "Cleaning up bricks.", 0.9f + (float)progress / bricks.Length * 0.1f);
                }
        ...
        private static void AnalyzeMeshes(List<Matrix4x4> viewMatrices, List<Matrix4x4> projectionMatrices, List<Vector3> viewPositions, List<Vector3> viewDirections, List<Plane[]> viewFrustums, List<MeshInstance> instances, Dictionary<Part, int> partMeshCount, Dictionary<Part, int> partSurfaceMeshCount, bool allowInsideRemove, bool allowOutsideRemove, int lod)
        {
            ...
            if (allowInsideRemove || allowOutsideRemove)
            {
                foreach (var m in instances)
                {
                    ...
                    if (m.transparent)
                    {
                        var canRemove = ((m.type == MeshType.Legacy || m.type == MeshType.Shell || m.type == MeshType.ColourChange || m.type == MeshType.Knob) && allowOutsideRemove) || (m.type == MeshType.Tube && allowInsideRemove);
                        if (canRemove)
                        {
                    ...
                    if (m.pixelCount > 0)
                    {
                        if (m.type == MeshType.Legacy)
                        {
                            ...
                            if (allowOutsideRemove && m.pixelKnobCount < knobPixelThreshold)
                            {
                                optimizationLevel += 2;
                            }
                            if (optimizationLevel > 0)
                        ...
                        {
                            if (allowOutsideRemove && m.pixelCount < knobPixelThreshold)
                            {
                                m.mesh = null;
                                partMeshCount[m.part]--;
                            }
                        }
                        else if (m.type == MeshType.Tube)
                        {
                            if (allowInsideRemove && m.pixelCount < tubePixelThreshold)
                            {
                                m.mesh = null;
                                partMeshCount[m.part]--;
                            }
                    ...
                    {
                        if ((m.type == MeshType.Legacy || m.type == MeshType.Shell || m.type == MeshType.ColourChange || m.type == MeshType.Knob) && allowOutsideRemove)
                        {
                            m.mesh = null;
                            if (m.type == MeshType.Shell || m.type == MeshType.ColourChange)
                            {
                        ...
                        else if (m.type == MeshType.Tube && allowInsideRemove)
                        {
                            m.mesh = null;
                            partMeshCount[m.part]--;
                        }
