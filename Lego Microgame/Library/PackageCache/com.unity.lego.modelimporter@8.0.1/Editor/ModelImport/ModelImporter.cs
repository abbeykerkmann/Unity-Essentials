...
{
    /// <summary>
    /// Provides methods for instantiating models, model groups and bricks
    /// </summary>
    public class ModelImporter
    {
        /// <summary>
        /// Raised when model import has been started
        /// </summary>
        public static event Action modelImportStarted;

        /// <summary>
        /// Raised when model import has ended
        /// </summary>
        public static event Action modelImportEnded;
        ...
        /// <param name="importSettings">The import settings to use for this model group</param>
        /// <returns>Returns the model group game object</returns>
        public static GameObject InstantiateModelGroup(LXFMLDoc.BrickGroup group, int index, GameObject parent, string absoluteFilePath, string relativeFilePath, ref Dictionary<int, Brick> resultBricks, bool isSubGroup, ModelGroupImportSettings importSettings)
        {
            ...
            if (!isSubGroup)
            {
                ...
                groupComp.viewBasedOptimizations = ModelGroup.ViewBasedOptimizations.Everything;
        ...
        private static void DetectConnectivity(HashSet<Brick> onlyConnectTo, HashSet<Brick> ignoreForCollision = null)
        {
            ...
            foreach (var brick in onlyConnectTo)
            {
                ...
                {
                    var query = Connectivity.QueryConnections(field, out _, onlyConnectTo);
                    foreach(var (connection, otherConnection) in query)
                    {
                        var connections = BrickBuildingUtility.Connect(connection, otherConnection, onlyConnectTo, ignored, false);
        ...
        /// <param name="undo">Whether or not to record undo on this action</param>
        public static void InstantiateModelBricks(LXFMLDoc lxfml, DictionaryIntToModelGroupImportSettings importSettings, ref Dictionary<int, Brick> resultBricks, int groupNumber = -1, bool undo = false)
        {
            for (var i = 0; i < lxfml.bricks.Count; ++i)
            {
                ...
                foreach (var part in brick.parts)
                {
                    ...
                    var partExistenceResult = PartUtility.UnpackPart(part.partDesignId, brickLightmapped, brickLod);
                    ...
                    if (brickConnectivity)
                    {
                        ...
                        if (connectivityToInstantiate)
                        {
                            ...
                            bounds = connectivityComp.extents;
                            ...
                            foreach (var field in connectivityComp.planarFields)
                            {
                                ...
                                {
                                    PartImporter.MatchConnectionWithKnob(connection, partComp.knobs);
                                    PartImporter.MatchConnectionWithTubes(connection, partComp.tubes);
                                }
                            }
                        }
                        else
                        {
                            brickConnectivity = false;
                        }
                    }

                    PartImporter.SetMaterials(partComp, part.materials, partExistenceResult.existence == PartUtility.PartExistence.Legacy);
                    PartImporter.SetDecorations(partComp, part.decorations, partExistenceResult.existence == PartUtility.PartExistence.Legacy);
                    ...
                    if (group != null && importSettings[group.number].randomizeRotation)
                    {
                    ...
                    if (brickConnectivity)
                    {
                        var position = partGO.transform.localPosition;
                        bounds = new Bounds(position + bounds.center, bounds.size);
                        brickComp.totalBounds.Encapsulate(bounds);
                    }
                    else
                    {
                ...
                if (undo)
                {
                    Undo.RegisterCreatedObjectUndo(brickGO, "Creating brick");
                }
        ...
        /// <param name="filePath">The path of the model</param>
        /// <param name="pivot">The pivot to use for the model</param>
        /// <param name="importSettings">The import settings for the model</param>
        /// <returns>Returns the model game object</returns>
        public static GameObject InstantiateModel(LXFMLDoc lxfml, string filePath, Model.Pivot pivot, DictionaryIntToModelGroupImportSettings importSettings)
        {
            ...
            if (resultBricks.Count > 0)
            {
                ...
                if(ToolsSettings.AutoUpdateHierarchy)
                {
                    ...
                    ModelGroupUtility.RecomputeHierarchy(bricks, ModelGroupUtility.UndoBehavior.withoutUndo);
                }
            }
            ...
            {
                EditorUtility.DisplayProgressBar("Importing", "Computing pivot.", 0.9f);

                ModelGroupUtility.RecomputePivot(model, true, ModelGroupUtility.UndoBehavior.withoutUndo);
            }
            Undo.RegisterCreatedObjectUndo(parent, "Create Model");

            EditorUtility.ClearProgressBar();
            modelImportEnded?.Invoke();            
            return parent;
        }
        ...
        /// </summary>
        /// <remarks>
        /// Reimporting changes the existing model
        /// </remarks>
        /// <param name="lxfml">The lxfml document to use for reimporting</param>
        /// <param name="model">The model to reimport</param>
        /// <param name="previousPivot">The previous pivot type of the model</param>
        /// <param name="importSettings">The import settings of the model</param>
        public static void ReimportModel(LXFMLDoc lxfml, Model model, Model.Pivot previousPivot, DictionaryIntToModelGroupImportSettings importSettings)
        {
            ...
            if(removedGroups.Count > 0)
            {
                ...
                foreach(var group in removedGroups)
                {
                    ...
                    InstantiateModelBricks(lxfml, model.importSettings, ref resultBricks, number, true);
                    ...
                    Undo.RegisterCompleteObjectUndo(groupGO, "Created group");
                    if(importSettings[number].connectivity)
                    {
            ...
            foreach (var group in groups)
            {
                ...
                var originalPosition = group.transform.localPosition;
                var originalRotation = group.transform.localRotation;
                ...
                group.transform.localPosition = originalPosition;
                group.transform.localRotation = originalRotation;
            }
            ...
            if (ToolsSettings.AutoUpdateHierarchy)
            {
                ...
                ModelGroupUtility.RecomputeHierarchy(bricks, ModelGroupUtility.UndoBehavior.withUndo);
        ...
        /// <summary>
        /// Reimport a model group
        /// </summary>
        /// <remarks>
        /// On reimport the existing group is changed
        /// </remarks>
        /// <param name="lxfml">The lxfml document to reimport from</param>
        /// <param name="group">The group to reimport</param>
        /// <param name="importSettings">The import settings to use on reimport</param>
        /// <param name="detectConnectivity">Whether or not to detect connectivity</param>
        ...
        /// <summary>
        /// Sets the necessary state on a GameObject for static and lightmapping
        /// </summary>
        /// <param name="go">The GameObject</param>
        /// <param name="isStatic">Whether or not the GameObject should be static</param>
        /// <param name="lightmapped">Whether or not the GameObject will be lightmapped</param>
        /// <param name="recursive">Whether or not we should set the information recursively</param>
        public static void SetStaticAndGIParams(GameObject go, bool isStatic, bool lightmapped, bool recursive = false)
        {
            if (isStatic)
            {
                go.isStatic = true;
