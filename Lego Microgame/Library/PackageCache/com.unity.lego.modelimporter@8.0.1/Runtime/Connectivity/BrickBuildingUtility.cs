...
{
    /// <summary>
    /// Provides methods and fields for using the LEGO connectivity data within Unity
    /// </summary>
    public class BrickBuildingUtility
    {
        ...
        /// <summary>
        /// 5 LEGO units. <see cref="LU_1"/>
        /// </summary>
        public const float LU_5 = 5 * LU_1;

        /// <summary>
        /// 10 LEGO units. <see cref="LU_1"/>
        /// </summary>
        public const float LU_10 = 10 * LU_1;

        private const float axleDistanceTolerance = LU_1 * 2;
        private const float fixedDistanceTolerance = LU_10 * 2;

        /// <summary>
        /// Constant for cos of 80 degrees
        /// </summary>
        internal const float Cos80 = 0.173612f;

        /// <summary>
        /// Constant for cos of 45 degrees
        /// </summary>
        internal const float Cos45Epsilon = 0.69465837f; // Epsilon required to handle randomized rotations

        /// <summary>
        /// The furthest distance we cast a ray for building
        /// </summary>
        internal const float MaxRayDistance = 250.0f;

        // Defines the number of bricks to consider when finding best connection. Reduce to improve performance.
        /// <summary>
        /// Defines the number of bricks to consider when finding the best connection.
        /// </summary>
        /// <remarks>
        /// This can be reduced for performance reasons
        /// </remarks>
        internal const int DefaultMaxBricksToConsiderWhenFindingConnections = 3;

        /// <summary>
        /// Layer mask for colliders to ignore on collision checks
        /// </summary>
        internal static int IgnoreMask => ignoreMask;

        private static readonly Vector2[] connectionFieldOffsets =
            new Vector2[] {
        ...
        internal static Collider[] ColliderBuffer => colliderBuffer;
        internal static RaycastHit[] RaycastBuffer => raycastBuffer;
        ...
        /// <param name="LU">The LU value to align to. Default value <see cref="LU_5"/></param>
        ...
        /// <param name="maxDistance">The max distance to cast our ray</param>
        /// <param name="collidingHit">Out parameter for a raycast hit</param>
        /// <returns>Returns <see langword="true"/> if a collision was found, <see langword="false"/> otherwise</returns>
        ...
        /// <param name="localToWorld">The transformation from local to world space to move colliders</param>
        /// <param name="colliderBuffer">Pre-allocated buffer for outputting colliding colliders</param>
        /// <param name="hits">Out parameter signifying the amounf of colliding hits</param>
        /// <param name="ignoreList">Optional list of bricks to ignore in the collision check</param>
        /// <returns>Returns <see langword="true"/> if a collision is found, <see langword="false"/> otherwise</returns>
        internal static bool IsCollidingAtTransformation(Brick brick, Matrix4x4 localToWorld, Collider[] colliderBuffer, out int hits, ICollection<Brick> ignoreList = null)
        {
            hits = 0;
            foreach (var part in brick.parts)
            {
                var isColliding = Part.IsColliding(part, localToWorld, colliderBuffer, out hits, ignoreList);
                if (isColliding)
                {
                    return true;
                }
        ...
        /// <returns>Returns an array of 8 bounding corners in the given transformation</returns>
        public static Vector3[] GetBoundingCorners(Bounds bounds, Matrix4x4 transformation)
        {
            var min = bounds.min;
            var max = bounds.max;

            //FIXME: Could cache sub-computations
            var p0 = transformation.MultiplyPoint3x4(new Vector3(min.x, min.y, min.z));
            var p1 = transformation.MultiplyPoint3x4(new Vector3(max.x, min.y, min.z));
            var p2 = transformation.MultiplyPoint3x4(new Vector3(min.x, max.y, min.z));
            var p3 = transformation.MultiplyPoint3x4(new Vector3(min.x, min.y, max.z));

            var p4 = transformation.MultiplyPoint3x4(new Vector3(max.x, max.y, min.z));
            var p5 = transformation.MultiplyPoint3x4(new Vector3(max.x, min.y, max.z));
            var p6 = transformation.MultiplyPoint3x4(new Vector3(min.x, max.y, max.z));
            var p7 = transformation.MultiplyPoint3x4(new Vector3(max.x, max.y, max.z));
        ...
        /// <returns>Returns the AABB Bounds object for the bricks</returns>
        ...
        /// <returns>Returns the offset required to align the bricks above the plane</returns>
        private static Vector3 GetOffsetToGrid(Vector3 focusBrickPosition, HashSet<Brick> bricks, Ray ray, Vector3 pickupOffset, Matrix4x4 transformation)
        {
        ...
        ///<param name="prerotateOffset">The offset to move the bricks before rotation</param>
        ...
        /// <returns>Returns a list of fields that have been connected to</returns>
        ...
        /// <param name="bounds">The world space bounds of <paramref name="selectedBricks"/></param>
        /// <param name="result">Out parameter representing the connection results. Will at most have size <paramref name="maxTries"/></param>
        /// <param name="maxTries">Optional parameter for amount of tries in finding a good connection</param>
        /// <returns>Returns <see langword="true"/> if a connection was found, <see langword="false"/> otherwise</returns>
        public static bool FindBestConnection(Vector3 pickupOffset, HashSet<Brick> selectedBricks, Ray ray, Matrix4x4 viewTransform, Brick[] bricksToCheck, Bounds bounds, out ConnectionResult[] result, int maxTries = DefaultMaxBricksToConsiderWhenFindingConnections)
        {
            ...
            foreach (var brick in possibleBricks)
            {
                ...
                if (FindBestConnectionOnBrick(selectedBricks, selectedFields, pickupOffset, brick, ray, maxSqr, bounds, out ConnectionResult currentResult))
                {
        ...
        public struct ConnectionResult
        {
            ...
            /// <summary>
            /// The interaction between source and destination
            /// </summary>
            public Connection.ConnectionInteraction interaction;

            /// <summary>
            /// Intersection point for axle features
            /// </summary>
            public Vector3 intersectionPoint;

            /// <summary>
            /// Distance to connection usable in comparisons
            /// </summary>
            public float maxSqrDistance;

            /// <summary>
            /// The angle required to align src to dst
            /// </summary>
            public float angleToConnect;

            /// <summary>
            /// The axis used when aligning src to dst
            /// </summary>
            public Vector3 rotationAxisToConnect;

            /// <summary>
            /// The translation required to align src todst
            /// </summary>
            public Vector3 connectionOffset;

            /// <summary>
            /// Whether this connection is colliding
            /// </summary>
            ...
            /// </summary>
            /// <returns>Returns an empty result</returns>
            ...
            /// </summary>
            /// <returns>Returns <see langword="true"/> if the result is empty, <see langword="false"/> otherwise</returns>
            public bool IsEmpty()
            {
        ...
        private static bool CheckConnectionCandidate(ConnectionField sourceField, ConnectionField destinationField, HashSet<Brick> selectedBricks, Vector3 pickupOffset, Ray ray, float maxSqrDistance, Bounds bounds, out ConnectionResult result)
        {
            oldPositions.Clear();
            oldRotations.Clear();
            ...
            if (destinationField is AxleField destinationAxle && sourceField is AxleField sourceAxle)
            {
                ...
                if (valid)
                {
                    ...
                    result.colliding = Colliding(selectedBricks, bounds, selectedBrick.gameObject.scene.GetPhysicsScene(), ColliderBuffer, out _);
        ...
        private static bool FindBestConnectionOnBrick(HashSet<Brick> selectedBricks, HashSet<ConnectionField> selectedFields, Vector3 pickupOffset, Brick brick, Ray ray, float maxSqrDistance, Bounds bounds, out ConnectionResult result)
        {
            ...
            foreach (var part in brick.parts)
            {
                ...
                foreach (var (field, selectedField) in pairs)
                {
                    ...
                    if (CheckConnectionCandidate(selectedField, field, selectedBricks, pickupOffset, ray, maxSqrDistance, bounds, out ConnectionResult connectionResult))
                    {
        ...
        /// <summary>
        /// Check whether there is a connectivity rejection for a list of bricks
        /// </summary>
        /// <param name="bricks">The bricks to check</param>
        /// <param name="overlapCompensation">Compensation used for axle features</param>
        /// <param name="ignore">An optional brick to ignore</param>
        /// <returns>Returns <see langword="true"/> if there was a rejection, <see langword="false"/> otherwise</returns>
        ...
        /// <summary>
        /// Check if a set of bricks are colliding
        /// </summary>
        /// <param name="bricks">The bricks to check</param>
        /// <param name="bounds">The world space bounds of the bricks</param>
        /// <param name="physicsScene">The physics scene to check collision in</param>
        /// <param name="colliderBuffer">Pre-allocated buffer for outputting colliding colliders</param>
        /// <param name="hits">Out parameter that signifies the number of colliding hits</param>
        /// <param name="checkRejection">Whether or not to check for connectivity rejection as well as collision</param>
        /// <returns></returns>
        public static bool Colliding(ICollection<Brick> bricks, Bounds bounds, PhysicsScene physicsScene, Collider[] colliderBuffer, out int hits,  bool checkRejection = false)
        {
            // Do a broad collision check based on the selection bounds box, which will always contain all bricks.
            hits = physicsScene.OverlapBox(bounds.center, bounds.size * 0.5f, colliderBuffer, Quaternion.identity, IgnoreMask, QueryTriggerInteraction.Ignore);
            if (hits == 0)
            {
                return false;
            }
            ...
            {
                // Rejection check
                if (CheckRejection(bricks, out _))
                {
                    return true;
                }
            }
            ...
            foreach (var brick in bricks)
            {
                if (IsCollidingAtTransformation(brick, brick.transform.localToWorldMatrix, colliderBuffer, out hits, bricks))
                {
                    return true;
                }
            }
